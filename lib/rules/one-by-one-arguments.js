  ...
  create(context) {
    ...
    return {
      CallExpression(node) {
        ...
        const isViolated = node.arguments.some(
          /**
           * @returns {boolean}
           */
          function isViolated(arg) {
            if (arg.type === "ArrayExpression") {
              return arg.elements.some(element => {
                return element ? isViolated(element) : false
              });
            }
      ...
      function fix(fixer) {
       ...
       function toMultiString(node, wrapAsArray = false) {
         if (node.type === "ArrayExpression") {
           return node.elements.flatMap((e) => e ? toMultiString(e) : []);
         }
